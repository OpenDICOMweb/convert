...
void main() {
	Server.initialize(name: 'read_files_old.dart', level: Level.debug3);
  print('Read Files');
...
bool readCheck(File file, int fileNo, {int reps = 1, bool fmiOnly = false}) {
	final rds = EvrByteReader.readFile(file);
  if (rds == null) {
...
bool readWriteCheck(File file, {int reps = 1, bool fmiOnly = false}) {
  ...
  final rds0 = EvrByteReader.readBytes(bytes0, path: file.path);
