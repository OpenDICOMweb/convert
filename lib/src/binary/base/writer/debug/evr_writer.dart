...
class EvrWriterDebug extends EvrWriter {

  EvrWriterDebug(RootDataset rds,
  {String path,
  ...
  : super(rds, path: path, eParams: eParams, outputTS: outputTS, minLength: minLength, 
              reUseBD: reUseBD, elementOffsetsEnabled: elementOffsetsEnabled, 
  ...
  void _writeShortEvr(Element e) {
    ...
    wb
      ..code(e.code)
      ..uint16(e.vrCode)..uint16(e.vfLength);
    __writeValueField(e);
  ...
  void _writeEncapsulatedPixelData(Element e) {
    ...
    if (e.vfLengthField == kUndefinedLength) {
      for (final bytes in e.fragments.fragments) {
        wb
          ..uint32(kItem32BitLE)..uint32(bytes.lengthInBytes)
          ..bytes(bytes);
      }
      wb..uint32(kSequenceDelimitationItem32BitLE)..uint32(0);
...
/*
  ...
  void writeExistingFmi(RootDataset rootDS, {bool cleanPreamble = true}) {
    writePrefix(rootDS, cleanPreamble);
...
//TODO: redoc
  ...
  @override
  bool writePrefix(RootDataset rds, bool cleanPreamble) {
